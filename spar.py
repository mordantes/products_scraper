    ...
    for item in menu_block:
        if item.select_one('a>span.name') is not None:
            ...
            if sub_menu_block is not None:
                ...
                menu_list = list(chain(
                        menu_list, 
                        [dict(
                            sub_category= _item.select_one('span').get_text() if _item.select_one('span') is not None else 'spar_sub_cat', 
                            href = URL + _item.attrs['href']+ ALL, 
                            category = item.select_one('a>span.name').get_text()
                        ) for _item in sub_menu if _item is not None
                        ]))
            else:
...
def parse_cards(curr_page, item):
    ...
    for product in product_cards:
        ...
        prices = tuple(
            [price_element.get_text() for price_element in product.select('span.values_wrapper>span.price_value')]
        )
        _result.append(
            dict(
                name= name,
